...
function mostrarMenuSugerencias() {
    tituloPagina.innerHTML = ""
    divPeliculas.innerHTML = ""
    let subMenuSugerencias = document.getElementById("subMenu")
    subMenuSugerencias.setAttribute("class", "subMenu")
    subMenuSugerencias.innerHTML = `
        <div>Peliculas sugeridas por nuestros usuarios para su futura inclusión</div>
        <section class="opciones">
            <article>
                <button id="sugerirTitulo">Sugerir un título</button>
                <input type="text" id="inputSugerencia">
            </article>
            <article>
                <button id="botonMostrarSugerencias">Ver sugerencias</button>
            </article>
        </section>`
    let botonMostrarSugerencias = document.getElementById("botonMostrarSugerencias")
    botonMostrarSugerencias.addEventListener("click", mostrarSugerencias)

    const guardarSugerenciaBtn = document.getElementById("sugerirTitulo")
    let tituloSugerido = document.getElementById("inputSugerencia")
    guardarSugerenciaBtn.addEventListener("click", () => guardarTituloSugerido(tituloSugerido.value))
}

function guardarTituloSugerido(tituloSugerido) {
    sugerencias.push(tituloSugerido)
}

function mostrarSugerencias() {
    
    tituloPagina.innerText = "Lista de películas sugeridas"
    divPeliculas.innerHTML = ""
    divPeliculas.setAttribute("class", "listaSugerencias")
    sugerencias.forEach((pelicula) => {
        let nuevaPelicula = document.createElement("div")
        nuevaPelicula.innerHTML = `<article class="cadaSugerencia">
                                            ${pelicula}
                                    </article>`
...
// operador ternario para mostrar los resultados de la búsqueda
...
// Esta función muestra la lista completa. Utiliza el acceso condicional a la propiedad de un objeto (el atributo pais) para avisar la procedencia o avisar que la misma no se indica
function mostrarListaCompleta() {
    ...
    filmoteca.forEach((pelicula) => {
        ...
        nuevaPelicula.innerHTML = `<article class="card">
                                        ...
                                        <p>estrenada en ${pelicula.anio}</p>
                                        <p>dirigida por ${pelicula.director}</p>
                                        <p>País: ${pelicula?.pais || "no indicado"}</p>
                                        <img src="${pelicula.afiche}">
...
// Esta función agrega al array favoritas. Un operador AND impide que se agregue de nuevo una película previamente incluida
...
// Esta función utiliza un operador ternario para avisar al usuario que aún no se eligieron películas o llama a la función para mostrar la lista
function favoritasONo() {
    favoritas.length == 0 ? (tituloPagina.innerText = "No hay películas favoritas elegidas") : mostrarFavoritas()
}

// Esta función utiliza desestructuración en parámetro del objeto recibido, parsea y muestra la lista de favoritas
...
let sugerenciasBtn = document.getElementById("sugerencias")
...
sugerenciasBtn.addEventListener("click", mostrarMenuSugerencias)

// clase Peliculas, a partir de la cual armar los objetos
class Peliculas {
    constructor(titulo, anio, director, afiche, pais) {
        this.titulo = titulo,
        this.anio = anio,
...
let pelicula01 = new Peliculas("La morada del diablo", 1896, "Melies", "./media/manoir-melies-1896.jpg", "Francia")
let pelicula02 = new Peliculas("Las cuatrocientas farsas del diablo", 1906, "Melies", "./media/farces-diable-melies-1906.jpg", "Francia")
let pelicula03 = new Peliculas("El estudiante de Praga", 1913, "Rye", "./media/rye-prag-1913.jpg")
let pelicula04 = new Peliculas("El Golem", 1914, "Wegener", "./media/der-golem-1914.jpg", "Alemania")
...
let pelicula09 = new Peliculas("Páginas del diario de Satán", 1921, "Dreyer", "./media/satans-dreyer-1921.jpg", "Dinamarca")
let pelicula10 = new Peliculas("Haxan", 1921, "Christensen", "./media/haxan-1921.jpg", "Dinamarca")
...
// array con lista de películas ya sugeridas
peliculasYaSugeridas = ["El fin del mundo", "El ángel azul", "Frankenstein"]

// array de sugerencias con un spread del array de las películas ya elegidas
let sugerencias = [...peliculasYaSugeridas]

// operador OR para definir si favoritas se trata de un array vacío o no
